...
-- Baseado no schema Drizzle do projeto
...
ALTER TABLE "designersInstrucionais" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "cursosDisciplinas" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "ofertasDisciplinas" ENABLE ROW LEVEL SECURITY;
ALTER TABLE videoaulas ENABLE ROW LEVEL SECURITY;
ALTER TABLE "historicoImportacoes" ENABLE ROW LEVEL SECURITY;

-- Políticas RLS: permitir leitura pública, escrita apenas para admins
-- (Você pode ajustar conforme necessário)

-- Leitura pública para todas as tabelas
CREATE POLICY "Allow public read access" ON cursos FOR SELECT USING (true);
CREATE POLICY "Allow public read access" ON disciplinas FOR SELECT USING (true);
CREATE POLICY "Allow public read access" ON professores FOR SELECT USING (true);
CREATE POLICY "Allow public read access" ON "designersInstrucionais" FOR SELECT USING (true);
CREATE POLICY "Allow public read access" ON "cursosDisciplinas" FOR SELECT USING (true);
CREATE POLICY "Allow public read access" ON "ofertasDisciplinas" FOR SELECT USING (true);
CREATE POLICY "Allow public read access" ON videoaulas FOR SELECT USING (true);

-- Escrita apenas para admins (baseado em auth.uid())
-- Nota: Você precisará ajustar isso depois de configurar Supabase Auth
CREATE POLICY "Allow admin write access" ON cursos FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE users."openId" = auth.uid() AND users.role = 'admin')
);
CREATE POLICY "Allow admin write access" ON disciplinas FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE users."openId" = auth.uid() AND users.role = 'admin')
);
CREATE POLICY "Allow admin write access" ON professores FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE users."openId" = auth.uid() AND users.role = 'admin')
);
CREATE POLICY "Allow admin write access" ON "designersInstrucionais" FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE users."openId" = auth.uid() AND users.role = 'admin')
);
CREATE POLICY "Allow admin write access" ON "cursosDisciplinas" FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE users."openId" = auth.uid() AND users.role = 'admin')
);
CREATE POLICY "Allow admin write access" ON "ofertasDisciplinas" FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE users."openId" = auth.uid() AND users.role = 'admin')
);
CREATE POLICY "Allow admin write access" ON videoaulas FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE users."openId" = auth.uid() AND users.role = 'admin')
);
CREATE POLICY "Allow admin write access" ON "historicoImportacoes" FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE users."openId" = auth.uid() AND users.role = 'admin')
);
